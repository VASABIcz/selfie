...
void init_scanner () {
  SYMBOLS = smalloc((SYM_NOT + 1) * sizeof(uint64_t*));
  ...
  if (find_next_character() != CHAR_EOF) {
    if (symbol != SYM_DIVISION) {
      ...
      if (is_letter(character)) {
        ...
        symbol = SYM_OR;
      } else if (character == CHAR_SQUIGLY) {
        get_character();
...
uint64_t compile_factor() {
  ...
  if (negate) {
    ...
    emit_xori(current_temporary(), current_temporary(), UINT64_MAX);
  }
...
void do_xori() {
  ...
  if (rd != REG_ZR) {
    ...
    if (imm == UINT64_MAX)
      ...
      printf("xori does not suport immediates other that 0 / -1");
